...
{
public:
  Wood(const glm::vec3 &minBound, const glm::vec3 &maxBound, const Material &mat)
  ...
  Intersect rayIntersect(const glm::vec3 &rayOrigin, const glm::vec3 &rayDirection) const override
  {
    ...
    {
    // Madera y
      Color c = loadTexture(std::abs(intersect.point.x - minBound.x), std::abs(intersect.point.z - minBound.z), "wood");
      ...
      // Madera z
      Color c = loadTexture(std::abs(intersect.point.x - minBound.x), std::abs(intersect.point.y - minBound.y), "wood");
      ...
      // Madera x
      Color c = loadTexture(std::abs(intersect.point.z - minBound.z), std::abs(intersect.point.y - minBound.y), "wood");
